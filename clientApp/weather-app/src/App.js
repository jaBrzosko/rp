...
function App() {
  ...
  const [overlayPanelsColor, setOverlayPanelsColor] = useState(initialColors);
  const [dataReady, setDataReady] = useState(false)
  const [isAnimationPlaying, setIsAnimationPlaying] = useState(false)
  const [currentIterationConst, setCurrentIterationConst] = useState(0)
  ...
  const increaseIteration = () => {
    currentIteration = currentIteration + 1 <= maxDaysInCurrentMonth ? currentIteration + 1 : currentIteration
    setCurrentIterationConst(currentIteration)
  }
  ...
  useEffect(() => {
    overlayPanelsColor.forEach((element, i) => {
      const index = i + 1;
  ...
  const handleLeftArrowClick = () => {
    ...
    if (currentHour === 0) {
      var newCurrentDay = currentDay - 1;
  ...
  const handleRestartToggleButton = () => {
      setCurrentDay(1)
      setCurrentHour(0)
      setCurrentIterationConst(0)
      currentIteration = 0
    if (isAnimationPlaying === false) {
      handleStartToggleButton()
    }
  }
    ...
    for(let i = 0; i < 12; ++i) {
      for (let j = 0; j < 12; ++j) {
        if (i === 0 || i === 11 || j === 0 || j === 11) {
          ...
          const temperatureString = data.points[dataIndex].date_temperature[currentIteration].temperature;
      ...
      <div className="container">
        ...
        <div className="image">
          ...
          <div className="overlay-grid">
            {overlayPanelsColor.map((color, index) => (
              <div
                key={index}
